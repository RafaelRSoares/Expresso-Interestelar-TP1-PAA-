...
void imprimirListaUI(Percurso* lista, int pecasTotais){
...
void on_button_ler_arquivo_clicked(){
    ...
    imprimirListaUI(&percurso, pecasTotais);
    char texto[256];
    GtkTextBuffer *buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(gtk_builder_get_object(builder, "text_saida")));
    GtkTextIter end;
    if(resultado == 1){
...
void on_button_facil_clicked(){
    ...
    char texto[100];
    sprintf(texto, "O seu mapa na dificuldade fácil foi gerado com o nome: %s", nomeArquivo);
...
void on_button_medio_clicked(){
    ...
    char texto[100];
    sprintf(texto, "O seu mapa na dificuldade médio foi gerado com o nome: %s", nomeArquivo);
    mensagemB("Mapa gerado", texto);
    gtk_stack_set_visible_child_name(stack, "view_escolher_Operação");
}
void on_button_dificil_clicked(){
    ...
    char texto[100];
    sprintf(texto, "O seu mapa na dificuldade difícil foi gerado com o nome: %s", nomeArquivo);
    mensagemB("Mapa gerado", texto);
    gtk_stack_set_visible_child_name(stack, "view_escolher_Operação");
}
...
void on_button_enviar_parametros_clicked(){
    ...
    char texto[100];
    sprintf(texto, "O seu mapa foi gerado com o nome: %s", nomeArquivo);
    mensagemB("Mapa gerado", texto);
    gtk_stack_set_visible_child_name(stack, "view_escolher_Operação");
}
